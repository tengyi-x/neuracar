...
`cloudPhysicsIO_blocks_libcachesim_baselines.csv`)
NeuraCaR vs. LRU/LFU/ARC/LeCaR/LHD/LRB on `twitter_cluster52` directly. `cloudPhysicsIO` is a
...
one fixed size" assumption, so its raw form only has libCacheSim baseline numbers (no NeuraCaR
column). To still get a NeuraCaR comparison on it, we chunked it into fixed 4096-byte blocks
(`../scripts/chunk_block_trace.py`) and reran everything at that granularity — see
`cache_experiments_summary.md` for that comparison.
